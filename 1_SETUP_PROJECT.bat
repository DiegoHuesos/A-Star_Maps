...
:: Check if node.js is installed
IF NOT EXIST "%nodejsPath%\node.exe" ECHO node.js is not installed... Please install first... && GOTO INSTALL
::IF NOT %nodejsTask% == 0 GOTO PREPARE
start cmd /k call npm install --save express body-parser express-handlebars googlemaps
timeout /t 12
start cmd /k call APP_JS.bat
timeout /t 1
